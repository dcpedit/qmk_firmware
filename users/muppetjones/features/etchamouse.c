...
typedef struct {
    ...
    uint16_t timer_x : 16;
    uint16_t timer_y : 16;
    uint16_t elapsed_x : 16;
    uint16_t elapsed_y : 16;
} key_tracker_t;
 ...
 */
static int8_t move_unit(key_tracker_t *tracker, uint8_t index) {
    if (index == 0 && 0 == tracker->count_x) return 0;
    else if (index == 1 && 0 == tracker->count_y) return 0;

    const uint16_t elapsed = index == 0 ? tracker->elapsed_x : tracker->elapsed_y;

    const uint16_t modifier = TAPPING_TERM_MOUSE_ENCODER < elapsed ? 1 : (TAPPING_TERM_MOUSE_ENCODER - elapsed) >> 1;
    //uint16_t       speed    = MOUSEKEY_INITIAL_SPEED + MOUSEKEY_MOVE_DELTA * modifier * (tracker->count >> 1);
    uint16_t       speed    = MOUSEKEY_INITIAL_SPEED + MOUSEKEY_MOVE_DELTA * modifier;
 ...
 * @return None.
 */
static void update_tracker(key_tracker_t *tracker, bool pressed, bool clockwise, uint8_t index) {

    if (index == 0) {
        tracker->elapsed_x = timer_elapsed(tracker->timer_x);

        if (pressed) {
            tracker->timer_x = timer_read();
            tracker->count_x += 1;
            tracker->clockwise = clockwise;
        } else if (TAPPING_TERM_PERSISTENCE < tracker->elapsed_x) {
            tracker->count_x = 0;
        } else if (TAPPING_TERM_MOUSE_ENCODER < tracker->elapsed_x) {
            tracker->count_x >>= 1;
        }
    }
    else if (index == 1) {
        tracker->elapsed_y = timer_elapsed(tracker->timer_y);

        if (pressed) {
            tracker->timer_y = timer_read();
            tracker->count_y += 1;
            tracker->clockwise = clockwise;
        } else if (TAPPING_TERM_PERSISTENCE < tracker->elapsed_y) {
            tracker->count_y = 0;
        } else if (TAPPING_TERM_MOUSE_ENCODER < tracker->elapsed_y) {
            tracker->count_y >>= 1;
        }
    }
}
...
bool encoder_update_mouse(uint8_t index, bool clockwise) {
    ...
    update_tracker(&tracker_x, 0 == index, clockwise, index);
    update_tracker(&tracker_y, 1 == index, clockwise, index);

    curr_report.x += move_unit(&tracker_x, index);
    curr_report.y += move_unit(&tracker_y, index);
